...
var MESSAGES = Symbol('ReactIntlMessages'); // combined default messages object

var combinedMessages = {}; // reversed combined default messages temp object, to detect duplicate default messages
...
function _default(_ref) {
  ...
  function sortObjByKeys(obj) {
    return Object.keys(obj).sort().reduce(function (acc, curKey) {
    ...
    descriptors.forEach(function (item) {
      ...
      */
      ...
      if (combinedMessages[item.id] && combinedMessages[item.id] !== item.defaultMessage) {
        throw file.path.buildCodeFrameError('[React Intl] Duplicate id with different `defaultMessage`s!\n' + JSON.stringify({
          id: item.id,
          defaultMessage: combinedMessages[item.id]
        }) + '\n' + JSON.stringify({
          id: item.id,
      ...
      */
      ...
      if (rcdm[item.defaultMessage]) {
        throw file.path.buildCodeFrameError('[React Intl] Duplicate `defaultMessage` with different `id`s!\n' + JSON.stringify({
          id: rcdm[item.defaultMessage],
          defaultMessage: item.defaultMessage
        }) + '\n' + JSON.stringify({
          id: item.id,
          defaultMessage: item.defaultMessage
      ...
      combinedMessages[item.id] = item.defaultMessage; // set id to temp default messages object
  ...
  return {
    ...
    post: function post(file) {
      ...
      if (opts.messagesDir && descriptors.length > 0) {
        ...
        var messagesFile; // check combine messages option
        ...
        if (opts.combineFile) {
          ...
          messagesFilename = p.join(opts.messagesDir, p.dirname(relativePath), basename + '.json');
